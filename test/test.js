...
describe('Classes:',() => {
  ...
  describe('class Function', () => {
    ...
    it('вычисление последовательности Фибоначчи (fib90)', () => {
      ...
      functions[`fib0`] = new Func('fib0', id1);
      functions[`fib1`] = new Func('fib1', id2);

      for (let i = 2; i <= 90; i++){
        functions[`fib${i}`] = new Func (`fib${i}`, functions[`fib${i-1}`], functions[`fib${i-2}`], '+');
      }
      
      assert.strictEqual(functions[`fib50`].getValue(), 12586269025 );
      assert.strictEqual(functions[`fib90`].getValue(), 2880067194370816120 );
    });
